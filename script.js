...
const revealSections = document.querySelectorAll(
  "#services, #about, .reviews-section, #contact"
);
...
// Image overlay / click-to-expand interaction
const detailImages = document.querySelectorAll(".feature-image, .pd-image");
const imageOverlay = document.getElementById("image-overlay");
const overlayImg = document.getElementById("overlay-img");

if (detailImages.length && imageOverlay && overlayImg) {
  detailImages.forEach(image => {
    image.addEventListener("click", () => {
      overlayImg.src = image.src;
      overlayImg.alt = image.alt || "Expanded image";
  ...
  imageOverlay.addEventListener("click", (event) => {
    if (event.target === imageOverlay || event.target.classList.contains("overlay-backdrop")) {
      closeOverlay();
    }
  });
